...
description: >-
  ...
  worktree), resolves the target project, and executes the workflow's DAG
  wave by wave via the Skill / Task / Bash tools, tracking per-step state
  under a ULID run directory. Invoked as `/wise-workflow-run`. Use when
  the user says "run the workflow", "start a workflow", "kick off
...
```bash
DEF=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" locate-def "<workflow-name>")
```
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" probe-requires "$DEF"
```
...
```bash
RUN_ID=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" new-ulid)
RUNS_ROOT="$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" runs-root)"
...
```bash
SESSION_ID=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" current-session-id)
```
...
```bash
SESSION_LABEL=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" \
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" \
...
```bash
STATE=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" \
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" prune-runs
```
...
```bash
eval "$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" \
  get-preflight "$DEF")"
```
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" list-inputs "$DEF"
```
...
2. Validate + extract via the engine — empty strings for regexes
   ...
   ```bash
   CLEAN=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" \
...
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" start-run "$STATE" "$CTX"
```
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" next-wave "$DEF" "$STATE"
```
...
```
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" resolve-team "$DEF" "<step.id>"
```
...
- **`mode: auto`** → pick the best-fit roster role. Cache the roster **once
  ...
  `python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" list-agents` once,
  ...
  2. `TaskCreate` the step's goal, then spawn ONE background worker:
     ...
     Agent({
       ...
       prompt: "<rendered def.prompt>"
               ...
               + "python3 \"${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py\" worker-heartbeat \"<run.dir>\" \"<step.id>-w1\" \"<phase>\" \"<step.id>\""
     })
     ```
...
- `type: ask`:
  ...
  **Shape B — binary choice** (`confirm_label` is declared). The
  ...
  ```bash
  python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" record-output \
    "$STATE" "<def.output>" "<answer>"
  python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" update-step \
...
2. For each step in the wave, in the same message:
   ...
   b. Append the full tool output to the step's log file. Use the
      ...
      ```bash
      python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" write-log \
        "$RUN_DIR" "<step.id>" "<step-run-ulid>" <<'WISE_LOG_EOF'
      <the full step output — subagent final message / bash stdout+stderr / etc.>
      WISE_LOG_EOF
      ```

      The `Bash(${WISE_PLUGIN_ROOT}/scripts/workflows.py:*)` grant
