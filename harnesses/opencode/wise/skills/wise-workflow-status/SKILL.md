...
> **Shared-file resolution:** `${WISE_PLUGIN_ROOT}` defaults to `${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode` — where `./install.sh opencode` puts this pack. Export `WISE_PLUGIN_ROOT` only to override.
...
- **`run-id` absent** (list every run):
  ```bash
  python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" list-runs "$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" runs-root 2>/dev/null)" 2>/dev/null || true
  ```
- **`run-id` present** — validate the ULID shape FIRST (26-char
  ...
  Then:
  ```bash
  python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" dump-state "$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/scripts/workflows.py" runs-root 2>/dev/null)/<run-id>/state.yaml" 2>/dev/null || true
  ```
