...
Follow `${WISE_PLUGIN_ROOT}/references/subject-drafting.md` §1 — the
...
echo "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/opencode}/references/pr/templates/pr-template.md"
```
...
- When the template is the bundled fallback
  (`${WISE_PLUGIN_ROOT}/references/pr/templates/pr-template.md`),
