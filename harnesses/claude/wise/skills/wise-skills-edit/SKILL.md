...
description: >-
  ...
  `harnesses/claude/wise/skills/`. Refuses the `wise` natural-language helper
...
The **`wise` natural-language helper** (`harnesses/claude/wise/skills/wise/SKILL.md`)
...
- `skill-name` (string, required) — the directory name of the skill
  to edit under `harnesses/claude/wise/skills/`. Must not equal `wise` (the
...
- Literal value `wise` — refuse with the helper-protection message:
  ...
  Claude Code session and edit harnesses/claude/wise/skills/wise/SKILL.md
  directly.
  ```
...
- Do **not** write or edit files yourself. `skill-creator` owns
  the file I/O.
- Do **not** allow editing of `harnesses/claude/wise/skills/wise/SKILL.md` —
