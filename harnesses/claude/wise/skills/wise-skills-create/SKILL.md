...
description: >-
  ...
  `harnesses/claude/wise/skills/<name>/`. The plugin hosts two skill shapes:
...
briefing pre-scoped to `harnesses/claude/wise/`, plus any free-form intent the
...
- `skill-name` (string, required) — the kebab-case identifier of the
  ...
  directory name under `harnesses/claude/wise/skills/`. By convention wise
...
`WISE_PLUGIN_ROOT` = `$REPO_ROOT/harnesses/claude/wise`.
...
- Do **not** create or edit anything outside `harnesses/claude/wise/` — this
