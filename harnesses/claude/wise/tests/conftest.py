...
WORKFLOWS_PATH = REPO / "harnesses" / "claude" / "wise" / "scripts" / "workflows.py"

_spec = importlib.util.spec_from_file_location("workflows", WORKFLOWS_PATH)
if _spec is None:
    raise RuntimeError(f"cannot load workflows module from {WORKFLOWS_PATH}")
if _spec.loader is None:
    raise RuntimeError(f"no loader for workflows module at {WORKFLOWS_PATH}")
