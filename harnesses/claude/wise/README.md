...
procedure in [`AGENTS.md`](./AGENTS.md). `harnesses/claude/wise/agents/*.md` is the
...
`harnesses/claude/wise/skills/<name>/`. You can even **migrate an existing
...
- **MCP server** → add to `harnesses/claude/wise/.mcp.json`. Claude Code
...
- **Adding an action** is a single-step operation: drop a skill
  directory into `harnesses/claude/wise/skills/<name>/` with `argument-hint:`
