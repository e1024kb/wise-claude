...
```bash
STATUS="$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/cursor}/scripts/init-registry.py" check 2>/dev/null || true)"
case "$STATUS" in
  INIT:ok) : ;;
  *)
    echo "Tip: run /wise-init to cache dep probe results and speed up future runs." >&2
    bash "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/cursor}/scripts/bootstrap-deps.sh"
    ;;
esac
```
...
- **prompt:**
  ...
  - **Agent (optional).** `AskUserQuestion`: `Which subagent runs this
    ...
    - `Force a specific role` — list the roster (run
      `python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/cursor}/scripts/workflows.py" list-agents`
