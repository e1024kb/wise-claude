...
this file in `${WISE_PLUGIN_ROOT}/references/pr/`; read them from
there.
...
- Commit using the `commit-from-fix.md` fragment with
  ...
  (`${WISE_PLUGIN_ROOT}/references/pr/commit-from-fix.md`) via the
  Read tool.
...
```
Read: ${WISE_PLUGIN_ROOT}/references/pr/handle-human-comments.md
```
...
```
Read: ${WISE_PLUGIN_ROOT}/references/pr/handle-bot-reviews.md
```
...
```
Read: ${WISE_PLUGIN_ROOT}/references/pr/handle-sonar-issues.md
```
