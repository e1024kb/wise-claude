...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" worker-heartbeat \
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" supervise-config
```
...
Monitor({
  ...
  command:
    'RUN_DIR="<run.dir>"; EXP="<worker-1,worker-2,…>"; POLL=<poll_secs>; \
     while true; do \
       python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" stale-workers "$RUN_DIR" "$EXP" 2>&1 \
...
message: |
  ...
  python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" worker-heartbeat "<run.dir>" "<name>" "<phase>" "<task-id>"
```
