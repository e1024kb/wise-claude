...
```bash
DEF=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" locate-def "<workflow-name>")
```
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" probe-requires "$DEF"
```
...
```bash
RUN_ID=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" new-ulid)
RUNS_ROOT="$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" runs-root)"
...
```bash
SESSION_ID=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" current-session-id)
```
...
```bash
SESSION_LABEL=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" \
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" \
...
```bash
STATE=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" \
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" prune-runs
```
...
```bash
eval "$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" \
  get-preflight "$DEF")"
```
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" list-inputs "$DEF"
```
...
2. Validate + extract via the engine — empty strings for regexes
   ...
   ```bash
   CLEAN=$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" \
...
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" start-run "$STATE" "$CTX"
```
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" next-wave "$DEF" "$STATE"
```
...
```
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" resolve-team "$DEF" "<step.id>"
```
...
- **`mode: auto`** → pick the best-fit roster role. Cache the roster **once
  ...
  `python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" list-agents` once,
  ...
  2. `TaskCreate` the step's goal, then spawn ONE background worker:
     ...
     Agent({
       ...
       prompt: "<rendered def.prompt>"
               ...
               + "python3 \"${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py\" worker-heartbeat \"<run.dir>\" \"<step.id>-w1\" \"<phase>\" \"<step.id>\""
     })
     ```
...
- `type: ask`:
  ...
  **Shape B — binary choice** (`confirm_label` is declared). The
  ...
  ```bash
  python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" record-output \
    "$STATE" "<def.output>" "<answer>"
  python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" update-step \
...
2. For each step in the wave, in the same message:
   ...
   b. Append the full tool output to the step's log file. Use the
      ...
      ```bash
      python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" write-log \
