...
description: >-
  ...
  auth login`, markitdown for file-to-markdown extraction) and cache the
  probe results so workflow runs skip the live check. Idempotent —
  re-running only prompts for gaps. Invoked as `/wise-init`. Use when the
  user says "init wise", "set up wise", "install wise deps", "first-time
  setup", "run the setup wizard", or types `/wise-init`.
---
...
`./install.sh pi --force` deletes it, which is exactly the
...
```bash
bash "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/init.sh" probe-python
```
...
```bash
bash "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/init.sh" probe-node
```
...
```bash
bash "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/init.sh" probe-gh
```
...
```bash
bash "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/init.sh" probe-markitdown
```
...
```bash
cat "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/.wise-version" 2>/dev/null || true
```
...
```bash
python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/init-registry.py" write '<the JSON blob>'
```
...
or after `./install.sh pi --force` (which deletes the cache by
design).
```
