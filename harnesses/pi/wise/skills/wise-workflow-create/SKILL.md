...
```bash
STATUS="$(python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/init-registry.py" check 2>/dev/null || true)"
case "$STATUS" in
  INIT:ok) : ;;
  *)
...
- **prompt:**
  ...
  - **Agent (optional).** `AskUserQuestion`: `Which subagent runs this
    ...
    - `Force a specific role` — list the roster (run
      `python3 "${WISE_PLUGIN_ROOT:-${WISE_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/wise}/harness/pi}/scripts/workflows.py" list-agents`
