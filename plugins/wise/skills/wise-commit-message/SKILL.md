...
description: >-
  ...
  against git — never runs `git commit`, `git add`, or anything that
  mutates the repo. Invoked as `/wise-commit-message` (bare alias) or
  `/wise:wise-commit-message` (canonical). Use when the user says "write
  a commit message", "draft a commit", "conventional commit", "prepare
  commit message", "one-line commit", or types `/wise-commit-message`.
argument-hint: "[--copy]"
allowed-tools: Read, AskUserQuestion, Bash(git:*), Bash(pbcopy:*), Bash(pbpaste:*), Bash(printf:*), Bash(command:*)
---
...
Pick the type per `${CLAUDE_PLUGIN_ROOT}/references/subject-drafting.md`
