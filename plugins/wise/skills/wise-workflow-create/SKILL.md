...
argument-hint: "<name>"
allowed-tools: Read, Write, AskUserQuestion, Bash(mkdir:*), Bash(test:*), Bash(bash:*), Bash(pwd:*), Bash(dirname:*), Bash(python3:*), Bash(${CLAUDE_PLUGIN_ROOT}/scripts/bootstrap-deps.sh:*), Bash(${CLAUDE_PLUGIN_ROOT}/scripts/init-registry.py:*), Bash(${CLAUDE_PLUGIN_ROOT}/scripts/workflows.py:*)
---
...
```bash
STATUS="$(python3 "${CLAUDE_PLUGIN_ROOT}/scripts/init-registry.py" check 2>/dev/null || true)"
case "$STATUS" in
  INIT:ok) : ;;
  *)
...
- None of the following already exist (both layouts are checked —
  ...
  - `${CLAUDE_PLUGIN_ROOT}/workflows/<name>/workflow.yaml` (bundled, folder)
  - `${CLAUDE_PLUGIN_ROOT}/workflows/<name>.yaml` (bundled, legacy flat)
  - `${CLAUDE_PLUGIN_DATA}/workflows/definitions/<name>/workflow.yaml` (user, folder)
  - `${CLAUDE_PLUGIN_DATA}/workflows/definitions/<name>.yaml` (user, legacy flat)
...
`plugins/wise/.claude-plugin/plugin.json` file AND a
...
d="$(pwd)"
while [ "$d" != "/" ]; do
  if [ -f "$d/plugins/wise/.claude-plugin/plugin.json" ] \
     && [ -f "$d/.claude-plugin/marketplace.json" ] \
     && [ -d "$d/plugins/wise/workflows" ]; then
    REPO_ROOT="$d"
    break
  fi
  d="$(dirname "$d")"
...
- If `REPO_ROOT` is set → `AskUserQuestion`:
  ...
  - Header: `Destination`
  - Options:
    - `Bundled (Recommended) — ships with plugin` — `Write to <REPO_ROOT>/plugins/wise/workflows/<name>/workflow.yaml. Tracked in git; seen by every wise installer after the next plugin release.`
    - `User-authored — private to your machine` — `Write to ${CLAUDE_PLUGIN_DATA}/workflows/definitions/<name>/workflow.yaml. Not tracked in any repo.`
...
- `mode = bundled` → `TARGET = "${REPO_ROOT}/plugins/wise/workflows/<name>/workflow.yaml"`
- `mode = user-authored` → `TARGET = "${CLAUDE_PLUGIN_DATA}/workflows/definitions/<name>/workflow.yaml"`
...
- **User-authored mode.** Ensure the workflow folder exists:
  ...
  ```bash
  mkdir -p "${CLAUDE_PLUGIN_DATA}/workflows/definitions/<name>"
  ```

  Write the YAML via the `Write` tool to `TARGET` (i.e.
  `${CLAUDE_PLUGIN_DATA}/workflows/definitions/<name>/workflow.yaml`).

- **Bundled mode.** `<REPO_ROOT>/plugins/wise/workflows/` always exists
  in a valid clone, but the `<name>/` subfolder does not. Create it:

  ```bash
  mkdir -p "<REPO_ROOT>/plugins/wise/workflows/<name>"
  ```

  Write the YAML via the `Write` tool to `TARGET` (i.e.
  `<REPO_ROOT>/plugins/wise/workflows/<name>/workflow.yaml`).
...
- Only two write destinations are valid: the user-authored path
  (`${CLAUDE_PLUGIN_DATA}/workflows/definitions/`) and — only when
  the detection in [§2.5](#25-detect-write-destination) matches and
  the user opts in — the bundled path
  (`<REPO_ROOT>/plugins/wise/workflows/`). Never write anywhere else.
