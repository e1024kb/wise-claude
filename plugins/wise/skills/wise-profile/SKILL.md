...
| level | meaning |
|---|---|
| `low` | cheapest run that keeps every gate: sonnet tiers for research / implementation (planning stays opus), minimal optional research, solo leads instead of panels, low retry caps |
...
- options:
  ...
  2. `low` — "Cheapest run that keeps every gate: sonnet tiers
     (planning stays opus), minimal optional research, solo leads,
     medium-effort reviews, low retry caps. Correctness rules
     unchanged."
...
e.g. `Profile low set for this session — sonnet tiers (planning stays
opus), minimal research, solo leads, medium-effort reviews. Reset with
/wise-profile medium.`
