...
- NEVER use a long dash - no em dash (—), no en dash (–), no other
  long-dash variant. They are among the strongest generated-text
  tells. Use a simple dash (-), a comma, a colon, or split into two
  sentences. Ranges too: "2-4 bullets", not "2–4".
...
- No link construct at all (plain-text surface) - the bare URL goes
  on its own line, never glued mid-sentence to punctuation, which
  breaks auto-linking
