...
3. Per task, after its edits: tidy the touched files (clarity, dead code, consistency; behaviour unchanged), stage only that task's files, commit once with a Conventional Commits subject. Use any verified ticket reference that is suitable as a scope; otherwise use an unscoped subject. One atomic commit per task. No AI attribution trailer.
