...
Both steps live in the shared routine — Read:
`${CLAUDE_PLUGIN_ROOT}/references/pr/sonar-fetch.md` — and follow it
end to end. It yields `SONAR_KEY` (+ `SONAR_KEY_GUESSED` when the key
is the last-resort `<org>_<repo>` guess) and exactly one fetch outcome
bucket: `OK (N issues)` / `OK (0 issues)` / `AUTH-FAIL` /
`FETCH-FAIL`. The issues-search endpoint is authoritative — no
separate key sanity probes (the rationale lives in that file).
