...
ticket is plannable — and, when it is not, how it turns each gap into a
question the right person can answer in one message. Read by:

- `skills/wise-grill/SKILL.md` §5 — the standalone `/wise-grill` gate.
- `workflows/ticket-plan/workflow.yaml` `gap-analysis` step — the
  interactive workflow's pre-plan gate.
- `workflows/ticket-auto/prompts/plan-ticket.md` §6 — the autonomous
  variant (see "Autonomous mode" below).
...
| 5 | **Design / UX** — expected screens, states, copy | user-facing tickets |
...
| 8 | **Data & edge cases** — inputs, volumes, error paths | logic-heavy tickets |
| 9 | **Verification** — how the change will be tested / demoed | always |
| 10 | **Risk / rollout** — migration, backwards compat, staged release | production-impacting tickets |
...
A dimension marked non-critical for this ticket kind is scored `n/a`.
...
over to someone's desk first?"* — not perfection. Nice-to-know
...
4. **Target a person, not the void.** Pick the addressee from the
   ...
   - Goal / intent → the **reporter** (they wanted it).
   ...
   - An attributed claim (Slack / comment) → the **person who made
     it**, to confirm it still holds.
   No name in evidence → target the **role** and say how to find them
   ("whoever owns the payments service").
