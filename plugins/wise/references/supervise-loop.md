...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/workflows.py" supervise-config
```
...
Monitor({
  ...
  command:
    'RUN_DIR="<run.dir>"; EXP="<worker-1,worker-2,…>"; POLL=<poll_secs>; \
     while true; do \
       python3 "${CLAUDE_PLUGIN_ROOT}/scripts/workflows.py" stale-workers "$RUN_DIR" "$EXP" 2>&1 \
...
message: |
  ...
  python3 "${CLAUDE_PLUGIN_ROOT}/scripts/workflows.py" worker-heartbeat "<run.dir>" "<name>" "<phase>" "<task-id>"
```
