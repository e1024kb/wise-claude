...
`references/legacy-conductor/` carry the low-frequency parts —
...
`${CLAUDE_PLUGIN_ROOT}/references/legacy-conductor/preflight.md` NOW — once for the whole run; it also carries
...
`${CLAUDE_PLUGIN_ROOT}/references/legacy-conductor/preflight.md` §5f.
...
follow `${CLAUDE_PLUGIN_ROOT}/references/legacy-conductor/preflight.md` §5h.
...
workflow inputs.** Follow `${CLAUDE_PLUGIN_ROOT}/references/legacy-conductor/preflight.md` §6b–§6c (already read
...
`${CLAUDE_PLUGIN_ROOT}/references/legacy-conductor/roster-resolution.md` — read it (with the model/effort rules
...
`${CLAUDE_PLUGIN_ROOT}/references/legacy-conductor/roster-resolution.md` — model is a native Task param (omit on
...
- `type: prompt` — **team** (`mode: team`), `type: supervised-prompt`,
  ...
  `${CLAUDE_PLUGIN_ROOT}/references/legacy-conductor/step-types.md` — read that file the FIRST time a wave
...
- `type: approval` / `type: ask`: dispatch per
  `${CLAUDE_PLUGIN_ROOT}/references/legacy-conductor/step-types.md` (read it the first time a wave contains one of
  these types).
