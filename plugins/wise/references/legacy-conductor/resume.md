...
`wise-workflow-run` [§5](run.md#5-generate-the-run-id-tag-the-session-write-stub-state)
...
From here on, behaviour is identical to `wise-workflow-run` [§10](run.md#10-main-loop) and [§11](run.md#11-finalise) —
...
Refer to `run.md` for the exact step dispatch rules
