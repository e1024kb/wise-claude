...
| `/wise-pr-watch-auto [<max-fix-attempts>] [--minutes <n>]` | Autonomous `/wise-pr-watch` — conducts the bundled `pr-watch` workflow: the engine polls CI and the review bots, fixes what they raise, pushes, requests one CodeRabbit verification review per pushed head when the repo leaves automatic incremental reviews off, runs wise's substitute review when a bot is stuck (only if the `substitute_review` pre-flight answer allows it) and merges when green and quiet (branch protection respected). Pre-flight asks harness, model and effort per phase (watch, fix, review, report); a human comment stands the run down. |
