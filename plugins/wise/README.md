...
| `/wise-exec-on-harness [--on <harness>] [--model <id>] [--effort <e>] [--mode ask\|auto\|full] [--p\|--prompt] <prompt>` | Run one free-form prompt as a headless child on any supported harness (`claude`, `codex`, `cursor`, `gemini`, `grok`) via the engine's `dispatch --relay`. Every omitted option (harness, model, effort, permission mode) is asked through a GUI/TUI picker; the prompt is the trailing free text or the value of `--p` / `--prompt`. Rejects a call with no prompt, or with options after the prompt. Gates relay to you; the child's answer is printed in full. |
