...
`engine.py` walks the plugin's `skills/` directory
(`plugins/wise/skills/`) and reads the `SKILL.md` frontmatter of every
skill directory it finds. Each skill is bucketed by the presence or
absence of the `argument-hint:` field:
