...
`wise_wait` blocks until at least one new event past `after`, or a gate, or terminal status, or `timeout_ms` (cap 600 000, default 240 000). It returns compact events only (E1); logs never cross the wire.
